...
function selectShowcase(index: number): void {
  if (index < props.project.showcase.length) activeShowcaseIndex.value = index;
}
    ...
    -->
      <div class="flex flex-col gap-0 md:gap-[200px]">
        ...
        <div ref="intro" class="flex flex-col gap-6 md:gap-20">
          <div
            class="flex flex-col gap-8 lg:flex-row lg:justify-between lg:gap-[342px]"
          >
            <div class="flex max-w-[612px] flex-col gap-4 md:gap-6">
              <h1
                class="text-[26px] font-semibold leading-normal text-neutral-900 md:text-[48px]"
              >
                {{ project.title }}
              </h1>
              <p
            ...
            <dl
              v-if="info.length > 0"
              class="grid w-full shrink-0 grid-flow-col grid-cols-2 grid-rows-2 justify-between gap-y-4 md:flex md:w-auto md:flex-col md:gap-6"
            >
              <div
                v-for="row in info"
                :key="row.label"
              ...
              >
                <dt class="flex items-center gap-2">
                  ...
                  />
                  <span
                    class="text-[14px] font-medium leading-[18px] text-neutral-1000 md:text-label-lg"
                    >{{ row.label }}</span
                  >
                </dt>
                <dd
                  class="max-w-[220px] text-[14px] leading-[18px] text-neutral-800 md:text-body-md"
                >
                  <a
                    v-if="row.href"
                    :href="row.href"
        ...
        >
          ...
          <div class="flex w-full max-w-[612px] flex-col gap-4 md:gap-6">
            <p
              class="text-[18px] font-medium leading-normal text-ink md:text-[32px]"
            >
          ...
          <ol class="grid gap-6 sm:grid-cols-2 lg:grid-cols-4">
            ...
            >
              ...
              <span class="flex flex-col gap-2">
                <span
                  class="text-[18px] font-medium text-neutral-900 md:text-[24px]"
                  >{{ card.title }}</span
                >
                <span
                  class="text-[14px] leading-normal text-neutral-800 md:text-[18px]"
        ...
        >
          <h2
            class="shrink-0 text-[22px] font-semibold text-neutral-900 md:text-[40px]"
          >
            {{ t("work.strategy") }}
          </h2>
          <ol class="grid w-full max-w-[860px] grid-cols-2">
            ...
            >
              ...
              <h3
                class="text-[16px] font-medium leading-normal text-gold md:text-[24px]"
              >
                {{ card.title }}
              </h3>
              <p
                class="text-[14px] leading-normal text-neutral-700 md:text-[18px]"
              >
                {{ card.description }}
              </p>
            </li>
        ...
        <!-- Deliverables — Figma 1294:4968; 3 across, textured cards -->
        <section
          ...
          <ol class="grid gap-6 sm:grid-cols-2 lg:grid-cols-3">
            ...
            >
              <img
                ...
                alt=""
                class="deliverables-card__background pointer-events-none absolute max-w-none opacity-60"
                aria-hidden="true"
              />
              <span
                class="relative z-10 text-[22px] font-medium leading-none text-gold md:text-[36px] md:leading-normal"
              >
                {{ String(i + 1).padStart(2, "0") }}
              </span>
              <span class="relative z-10 flex flex-col gap-2">
                <span
                  class="text-[18px] font-medium leading-normal text-neutral-900 md:text-[24px]"
                  >{{ card.title }}</span
                >
                <span
                  class="text-[14px] leading-normal text-neutral-800 md:text-[18px]"
        ...
        >
          <div class="md:flex md:items-center md:justify-between">
            ...
            <div
              class="absolute inset-x-0 top-[79px] flex justify-center gap-16 text-[14px] leading-none md:static md:inset-auto md:shrink-0 md:justify-start"
              role="tablist"
              :aria-label="t('work.content_types')"
            >
              <button
                v-for="(label, index) in showcaseFilters"
                v-show="index < project.showcase.length"
                ...
                :tabindex="activeShowcaseIndex === index ? 0 : -1"
                class="cursor-pointer whitespace-nowrap text-neutral-700 transition-colors"
                :class="
                  activeShowcaseIndex === index
                    ? 'font-medium text-ink underline decoration-from-font underline-offset-2'
                    : 'font-normal hover:text-ink'
                "
          ...
          <ul
            class="absolute inset-x-0 top-[160px] md:static md:mt-12 md:grid md:grid-cols-2 md:gap-6 lg:grid-cols-3"
          >
            <li
              ...
              role="tabpanel"
              :aria-labelledby="
                i < showcaseFilters.length ? `showcase-tab-${i}` : undefined
              "
              class="w-[262px] md:w-auto"
              :class="activeShowcaseIndex === i ? '' : 'hidden'"
            >
              <img
                :src="image.src"
                :srcset="image.srcset"
                ...
                width="400"
                height="500"
                class="h-[328px] w-full rounded-sm object-cover md:aspect-[4/5] md:h-auto"
              />
            </li>
          </ul>
        </section>
        ...
        >
          ...
          <ul
            class="mt-12 grid grid-cols-2 gap-3 md:mt-0 md:grid-cols-3 md:gap-6 lg:grid-cols-5"
          >
            <li
            ...
            >
              ...
              <span class="flex flex-col gap-2">
                <span
                  class="text-[18px] font-medium leading-normal text-neutral-900 md:text-title-lg"
                  >{{ stat.label }}</span
                >
                <span
                  class="latin-nums text-[22px] font-medium leading-normal text-gold md:text-[30px] md:leading-none"
                >
                  {{ stat.value }}
                </span>
              </span>
          ...
          <p
            v-if="project.resultsSummary"
            class="mt-8 whitespace-pre-line text-[16px] leading-normal text-neutral-800 md:mt-0 md:text-[24px] md:font-medium"
          >
      ...
      >
        <figure
          v-if="project.beforeAfter.before"
          class="flex flex-col gap-3 md:gap-6"
        >
          <figcaption
            class="text-center text-[20px] font-medium text-neutral-900 md:text-[28px] md:font-semibold"
          >
            {{ t("work.before") }}
          </figcaption>
          <img
        ...
        <figure
          v-if="project.beforeAfter.after"
          class="flex flex-col gap-3 md:gap-6"
        >
          <figcaption
            class="text-center text-[20px] font-medium text-neutral-900 md:text-[28px] md:font-semibold"
...
<style scoped>
.deliverables-card__background {
  height: 185%;
  inset-block-start: -9%;
  inset-inline-start: -119%;
  width: 265%;
}

.deliverables-card:nth-child(2) .deliverables-card__background {
  inset-block-start: 0;
  inset-inline-start: -83%;
}

.deliverables-card:nth-child(3) .deliverables-card__background {
  height: 206%;
  inset-block-start: -3%;
  inset-inline-start: -127%;
  width: 296%;
}

.deliverables-card:nth-child(4) .deliverables-card__background {
  height: 207%;
  inset-block-start: -4%;
...
.deliverables-card:nth-child(5) .deliverables-card__background {
  height: 207%;
  inset-block-start: 0;
...
.deliverables-card:nth-child(6) .deliverables-card__background {
  height: 207%;
  inset-block-start: -16%;
  inset-inline-start: -114%;
  width: 298%;
}
</style>
